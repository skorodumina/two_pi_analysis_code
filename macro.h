...
class macro {
	...
	TMacro* W_plot();
	void W_plot_write();
	TMacro* dif_crs_plot(Int_t j, Int_t i);
	void dif_crs_plot_write(Int_t j, Int_t i);
	...
	TMacro* ADC_plot(const char* name, Double_t xmin, Double_t ymax);
	TMacro* att_plot(const char* name);
